...
def wrap(text, font_px, max_w=CARD_W - 2 * MARGIN, cw=0.60):
    """Перенос по словам. cw — эмпирическая доля ширины символа от кегля.

    Считаем по САМОМУ ШИРОКОМУ шрифту стека: локально рендерит Helvetica, а в CI —
    DejaVu Sans, она заметно шире. Оценка под неё оставляет запас справа на маке
    и гарантирует, что в облаке строка не упрётся в край.
    """
